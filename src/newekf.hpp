...
class NewEKF { 
    ...
    private:
        ...
        static const auto QUEUE_ITEM_SIZE = sizeof(NewEKF::measurement_t);
        NewEKF::measurement_t _measurementsQueue[QUEUE_MAX_LENGTH];
        uint32_t _queueLength;

        void enqueue(const NewEKF::measurement_t * measurement) 
        {
            memcpy(&_measurementsQueue[_queueLength], measurement,
                    sizeof(NewEKF::measurement_t));
