...
class DshotBitbangEsc : public DshotEsc {
    ...
    private:
        ...
        static const timer_e USE_TIMER = TIMER_AUTO;

        bbPacer_t m_pacers[MAX_MOTOR_PACERS];  // TIM1 or TIM8
        uint8_t m_usedMotorPacers;

        bbPort_t m_ports[MAX_SUPPORTED_MOTOR_PORTS];
        uint8_t m_usedMotorPorts;
        ...
        void timebaseSetup(bbPort_t *bbPort, dshotProtocol_t dshotProtocolType)
        {
        ...
        bool motorConfig(IO_t io, uint8_t motorIndex, uint8_t output)
        {
    ...
    protected:
        ...
        virtual void deviceInit(void) override
        {
            ...
            for (int motorIndex = 0; motorIndex < m_motorCount; motorIndex++) {
                ...
                const timerHardware_t *timerHardware =
                    timerGetConfiguredByTag(m_motorPins[motorIndex]);

                const IO_t io = IOGetByTag(m_motorPins[motorIndex]);
        ...
        virtual void postInit(void) override
        {
            ...
            for (int motorIndex = 0; motorIndex < MAX_SUPPORTED_MOTORS && motorIndex <
                ...
                if (!motorConfig(
                            m_motors[motorIndex].io,
                            motorIndex,
                            m_motors[motorIndex].output)) { 
                    return;
                }
