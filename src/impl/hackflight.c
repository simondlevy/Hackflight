...
static void executeTask(task_t *task, timeUs_t currentTimeUs)
{
    ...
    taskNextStateTime = -1;
    if (taskExecutionTimeUs > (task->anticipatedExecutionTime >> TASK_EXEC_TIME_SHIFT)) {
        task->anticipatedExecutionTime = taskExecutionTimeUs << TASK_EXEC_TIME_SHIFT;
        ...
        uint32_t nextTargetCycles)
{
    ...
    int32_t gyroSkew = cmpTimeCycles(nextTargetCycles, gyroSyncTime()) % _desiredPeriodCycles;
...
static void updateDynamicTask(task_t * task, task_t ** selected, uint16_t * selectedPriority)
{
...
void hackflightFullInit(void)
{
    ...
    _schedLoopStartDeltaDownCycles = systemClockMicrosToCycles(1) / SCHED_START_LOOP_DOWN_STEP;
    _schedLoopStartDeltaUpCycles = systemClockMicrosToCycles(1) / SCHED_START_LOOP_UP_STEP;
    ...
    _taskGuardDeltaDownCycles = systemClockMicrosToCycles(1) / TASK_GUARD_MARGIN_DOWN_STEP;
    _taskGuardDeltaUpCycles = systemClockMicrosToCycles(1) / TASK_GUARD_MARGIN_UP_STEP;
