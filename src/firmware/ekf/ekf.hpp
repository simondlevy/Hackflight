    ...
    class EKF { 
        ...
        private:
                    ...
                    bool isFlying)  -> EKF
            {
                ...
                F(0,4) = (vy*ekf._R(2,2) - vz*ekf._R(2,1))*dt;

                F(0,5) = (-vx*ekf._R(2,2) + vz*ekf._R(2,0))*dt;

                F(0,6) = (vx*ekf._R(2,1) - vy*ekf._R(2,0))*dt;
                ...
                F(1,4) =  0;
                F(2,4) = -G*ekf._R(2,2)*dt;
                F(3,4) =  G*ekf._R(2,1)*dt;

                F(1,5) =  G*ekf._R(2,2)*dt;
                F(2,5) =  0;
                F(3,5) = -G*ekf._R(2,0)*dt;

                F(1,6) = -G*ekf._R(2,1)*dt;
                F(2,6) =  G*ekf._R(2,0)*dt;
                F(3,6) =  0;
