...
int i2cdevInit()
{
    tryToRestartBus(&deckBus);

    deckBus.isBusFreeSemaphore = xSemaphoreCreateBinaryStatic(
            &deckBus.isBusFreeSemaphoreBuffer);

    deckBus.isBusFreeMutex = xSemaphoreCreateMutexStatic(
            &deckBus.isBusFreeMutexBuffer);
        ...
        uint16_t len, uint8_t *data)
{
    ...
    createMessage16(&message, devAddress, memAddress, i2cRead, len, data);

    return messageTransfer(dev, &message);
}
        ...
        uint16_t len, const uint8_t *data)
{
    ...
    createMessage16(&message, devAddress, memAddress, i2cWrite, len, data);
