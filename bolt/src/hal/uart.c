...
bool uartGetCharWithTimeout(uint8_t *c, const uint32_t timeoutTicks)
{
  if (uartGetDataWithTimeout(1, c, timeoutTicks) > 0)
  {
    return true;
  }
...
bool uartGetCharWithDefaultTimeout(uint8_t *c)
{
  return uartGetCharWithTimeout(c, UART2_DATA_TIMEOUT_TICKS);
}

void uartGetchar(char * ch)
{
  uartGetData(1, (uint8_t*) ch);
}

int uartGetDataWithTimeout(size_t size, uint8_t * buffer, const uint32_t timeoutTicks) {
...
int uartGetData(size_t size, uint8_t * buffer) {
  size_t sizeLeft = size;
  while (sizeLeft > 0) {
    xStreamBufferSetTriggerLevel(rxStream, sizeLeft);
