...
payload msgtype vstate = (paysize, val00, val01, val02, val03, val04, val05) where
  ...
  val00 = if msgtype == 105 then c_receiverThrottle
          else if msgtype == 108 then rad2deg (phi vstate)
          else 0

  val01 = if msgtype == 105 then c_receiverRoll
          else if msgtype == 108 then (theta vstate)
          else 0

  val02 = if msgtype == 105 then c_receiverPitch
          else if msgtype == 108 then (psi vstate)
          else 0
