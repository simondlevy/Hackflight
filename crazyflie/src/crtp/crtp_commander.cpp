...
static void commanderCrtpCB(crtpPacket_t* pk);
...
static void commanderCrtpCB(crtpPacket_t* pk)
{
  ...
  if(pk->port == CRTP_PORT_SETPOINT && pk->channel == 0) {
    crtpCommanderOpenLoopDecodeSetpoint(&setpoint, pk);
    commander.setSetpoint(&setpoint, Commander::PRIORITY_CRTP);
