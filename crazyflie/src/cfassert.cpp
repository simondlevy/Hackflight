...
void assertFail(char *exp, char *file, int line)
{
  ...
  consolePrintf("SYSTEM: Assert failed %s:%d\n", file, line);
...
void printAssertSnapshotData()
{
  switch (currentType) {
    case SnapshotTypeNone:
      consolePrintf("SYSTEM: No assert information found\n");
      break;
    case SnapshotTypeFile:
      consolePrintf("SYSTEM: Assert failed at %s:%d\n", snapshot.file.fileName, snapshot.file.line);
      break;
    case SnapshotTypeHardFault:
      consolePrintf("SYSTEM: Hardfault. r0: %X, r1: %X, r2: %X, r3: %X, r12: %X, lr: %X, pc: %X, psr: %X\n",
    ...
    case SnapshotTypeText:
      consolePrintf("SYSTEM: Assert failed: %s\n", snapshot.text.text);
      break;
    default:
      consolePrintf("SYSTEM: Assert failed, but unknown type\n");
      break;
  }
}
...
bool cfAssertNormalStartTest(void) {
  bool wasNormalStart = true;
    ...
    currentType = snapshot.type;
		consolePrintf("SYSTEM: The system resumed after a failed assert [WARNING]\n");
    clearAssertData();
