...
static void resetUSB(void) {
  ...
  if (didInit == true) {
    // Empty queue
    while (xQueueReceiveFromISR(txQueue, &outPacket, &xTaskWokenByReceive) == pdTRUE)
      ;
  }
...
{
    while (xQueueReceive(rxQueue, in, portMAX_DELAY) != pdTRUE)
...
bool usbSendData(uint32_t size, uint8_t* data)
{
    ...
    return (xQueueSend(txQueue, &outStage, M2T(100)) == pdTRUE);
}
