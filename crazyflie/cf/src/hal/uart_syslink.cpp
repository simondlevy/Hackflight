...
STATIC_MEM_QUEUE_ALLOC(queue, 8, sizeof(syslinkPacket_t));
static bool queueReadyToReceive = false;
...
static volatile syslinkPacket_t slp = {0};
...
static void uartslkDmaInit(void)
{
...
static void uartslkResumeDma()
{
    ...
    if (dmaIsPaused)
    {
        ...
        UARTSLK_DMA_TX_STREAM->M0AR = 
            (uint32_t)&dmaTXBuffer[initialDMACount - remainingDMACount];
...
static void uartslkDmaRXIsr(void)
{
            ...
            (cksum[1] == dmaRXBuffer[slp.length + 1]))
    {
        ...
        if (!xQueueIsQueueFullFromISR(queue)) {
            if (queueReadyToReceive) {
                xQueueSendFromISR(queue, (void *)&slp, 
...
void uartslkIsr(void)
{
    ...
    else
    {
         ...
         */
        asm volatile ("" : "=m" (UARTSLK_TYPE->SR) : "r" (UARTSLK_TYPE->SR)); 
        asm volatile ("" : "=m" (UARTSLK_TYPE->DR) : "r" (UARTSLK_TYPE->DR)); 
    }
...
{
    queueReadyToReceive = true;
}

void uartslkGetPacketBlocking(syslinkPacket_t* packet)
{
