...
class MSPDriver(object):
    ...
    def __init__(self, root, canvas, port, baud):

        self.root = root
        self.canvas = canvas

        self.fmuport = serial.Serial(port, baud)
...
if __name__ == '__main__':
    ...
    if len(argv) < 3:
        print('Usage:   %s PORT BAUD' % argv[0])
    ...
    driver = MSPDriver(root, canvas, argv[1], int(argv[2]))
